...
pub fn solution() -> i64 {
    ...
    let (binary_z, decimal_z) = get_decimal_value_z(wires.clone());
    let (binary_y, decimal_y) = get_decimal_value_y(wires.clone());
    let (binary_x, decimal_x) = get_decimal_value_x(wires.clone());

    // add binary_str of y + x
    println!(
        "sum y+x {:b}",
        i64::from_str_radix(&binary_y, 2).unwrap() + i64::from_str_radix(&binary_x, 2).unwrap()
    );

    0
}

fn get_decimal_value_z(wires: HashMap<String, i32>) -> (String, i64) {
    let mut wires = wires;
    ...
    let binary_str = wires_ints
        ...
        .map(|(_, v)| v.to_string())
        .collect::<String>();
    println!("Binary value z: {}", binary_str);
    let decimal = i64::from_str_radix(&binary_str, 2).unwrap();
    println!("Decimal value z: {}", decimal);
    return (binary_str, decimal);
}

fn get_decimal_value_y(wires: HashMap<String, i32>) -> (String, i64) {
    let mut wires = wires;

    let keys_to_remove: Vec<String> = wires
        .iter()
        .filter(|(k, _)| k.chars().next().unwrap() != 'y')
        .map(|(k, _)| k.clone())
        .collect();
    ...
    let binary_str = wires_ints
        ...
        .map(|(_, v)| v.to_string())
        .collect::<String>();
    println!("Binary value y: {}", binary_str);
    let decimal = i64::from_str_radix(&binary_str, 2).unwrap();
    println!("Decimal value y: {}", decimal);
    return (binary_str, decimal);
}

fn get_decimal_value_x(wires: HashMap<String, i32>) -> (String, i64) {
    let mut wires = wires;

    let keys_to_remove: Vec<String> = wires
        .iter()
        .filter(|(k, _)| k.chars().next().unwrap() != 'x')
        .map(|(k, _)| k.clone())
        .collect();
    ...
    let binary_str = wires_ints
        ...
        .map(|(_, v)| v.to_string())
        .collect::<String>();
    println!("Binary value x: {}", binary_str);
    let decimal = i64::from_str_radix(&binary_str, 2).unwrap();
    println!("Decimal value x: {}", decimal);
    return (binary_str, decimal);
}
