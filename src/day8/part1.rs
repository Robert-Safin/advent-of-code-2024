    ...
    for (_, v) in hash.iter() {
        for i in 0..v.len() - 1 {
            for j in i + 1..v.len() {
                ...
                let anti_node_coords_one = (left.0 - col_diff, left.1 - row_diff);
                if anti_node_coords_one.0 >= 0
                ...
                let anti_node_coords_two = (right.0 + col_diff, right.1 + row_diff);
                if anti_node_coords_two.0 >= 0
